...
def plot(sims):
    ...
    ax.set_ylabel(r"$J_{\mathrm{BH}}/M_{\mathrm{BH}}^2$")
    ax.set_xlabel(time_xlabel())
    add_time_secondary_axis(ax)
    ax.set_ylim(*SPIN_YLIM)
    ax.grid()
    if final_times:
...
def main(argv=None):
    args = parser("Plot dimensionless BH spin.").parse_args(argv)
    setup(args)
    sims = load_sims(["spin_parameter"], names=args.sims)
