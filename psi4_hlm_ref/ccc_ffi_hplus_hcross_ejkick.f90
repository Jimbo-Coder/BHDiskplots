...
! 
   ...
   real*8 :: tret_start,tret_end, tret_i,temp1, F_GW, E_GW, J_GW, vx_GW, vy_GW, Px_GW,Py_GW
...
! spaced evenly in time with no repeated entries.
   ...
   write(*,*) "reading Psi4 file"
   ! Calculate the number of lines in the file and delta t
   read(1,*) t0 ! this should be t_start... 
   write(*,*) "read the time data?"
   n = 2
   do
   ...
   write(2,'("# t_coord t_sch t_retarded r_tortoise Re(rPsi4_22) Im(rPsi4_22)")')
   ...
   !!!! --> "rhpdlm" = "r * h_{plus} dot lm"
   !!!! --> "rhcdlm" = "r * h_{cross} dot lm"
   ...
   write(1,'("# t_retarded E_GW J_GW |v_GW| vx_GW vy_GW vz_GW F_GW")')
   do i=1,n-1
      F_GW = 0.d0 
      do ll=2,lmax-1
         do mm=-ll,ll
            ! here E_GW_i = Sum_{j = 0,i} (1/2) (F_GW_i + F_GW_{i+1}) 
            ! F_GW_i = Sum_{lm} (1/32π) r^2 (h^dot_+_lm^2 + h^dot_x_lm^2 )
            F_GW = F_GW + 1.d0/(16.d0*pi)*(rhpdlm(i,ll,mm)**2 + rhcdlm(i,ll,mm)**2)
      ...
      write(1,'(8ES25.15)') tret(1)+i*dt,E_GW,J_GW,sqrt(vx_GW**2+vy_GW**2+vz_GW**2),vx_GW,vy_GW,vz_GW,F_GW
   end do
   close(1)
